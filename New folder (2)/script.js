...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const sendMessage = () => {
        ...
        setTimeout(() => {
            ...
            if (hasIntent(intents.greeting)) {
                aiMsg.innerHTML = `Xin chào! 👋 Tôi là trợ lý ảo của AIStore. Bạn cần tìm trang phục tự mặc hay để làm quà tặng?`;
            }
                ...
                aiMsg.innerHTML = `📏 Đừng lo về kích cỡ! AIStore có hệ thống Virtual Try-on (Thử đồ ảo). Bạn có thể cho tôi biết chiều cao và cân nặng để AI của chúng tôi gợi ý chuẩn size (S, M, L, XL) cho bạn nhé.`;
            }
                ...
                aiMsg.innerHTML = `Không có chi! 😊 Rất hân hạnh được hỗ trợ bạn. Chúc bạn một ngày mua sắm tuyệt vời tại AIStore!`;
            }
    ...
    // 4. Tìm Kiếm Thông Minh (Smart Search & Filtering)
    ...
    const performSmartSearch = () => {
        ...
        // Tạo hiệu ứng mờ phân tích AI
        productGrid.style.opacity = 0.3;
        productGrid.style.transition = 'opacity 0.6s ease';
        ...
        setTimeout(() => {
            ...
            productCards.forEach(card => {
                ...
                // Thuật toán: Tách từ khóa và kiểm tra tỷ lệ match (giả lập Semantic Matching)
                const keywords = query.split(' ');
                let isMatch = keywords.some(kw => title.includes(kw) || category.includes(kw));

                // Đặc biệt: Phân tích intent giá cả (VD khách gõ "rẻ", "đắt", "sale")
                const isCheapQuery = ['rẻ', 'mềm', 'sale', 'rẻ nhất'].some(kw => query.includes(kw));
                let isPriceMatch = true;

                if (isCheapQuery) {
                    const priceText = card.querySelector('.price').textContent.replace('$', '');
                    const price = parseFloat(priceText);
                    isPriceMatch = price < 90; // Áo thun (45) & Jacket (89) là rẻ
                }

                if (isMatch || (isCheapQuery && isPriceMatch)) {
                    card.style.display = 'block';
                    card.style.animation = 'float-obj 0.8s ease-in-out'; // Animation búng lên
                    foundCount++;
                } else {
            ...
            if (foundCount === 0) {
                alert("Trợ lý AI phân tích: Không tìm thấy sản phẩm trùng khớp với ngữ cảnh của bạn. Đang hiển thị lại toàn bộ bộ sưu tập...");
                productCards.forEach(card => card.style.display = 'block');
                searchInput.value = '';
            }
        }, 800); // Fake delay 0.8s to simulate Neural Network processing
    };

    // Chuột click vào nút Magic
    ...
    try {
        const storedCart = localStorage.getItem('aiStoreCart');
    ...
    const saveCart = () => {
        localStorage.setItem('aiStoreCart', JSON.stringify(cart));
        renderCartUI();
    };
